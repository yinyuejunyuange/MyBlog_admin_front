<template>
  <div>
        ...
        <el-row :gutter="20">
          <el-col :span="6">
            ...
            <el-form-item label="作者名称">
              <el-input v-model="form.authorName" placeholder="输入作者名称" />
            </el-form-item>
          </el-col>
          ...
          <el-col :span="6">
            <el-form-item label="状态">
              <el-select v-model="form.status" placeholder="选择状态">
                <el-option label="保存中" value="保存中"></el-option>
                <el-option label="发布" value="发布"></el-option>
                <el-option label="审核中" value="审核中"></el-option>
                <el-option label="禁止查看" value="禁止查看"></el-option>
              </el-select>
            </el-form-item>
          </el-col>
        </el-row>
    ...
    <el-table :data="filteredBlogs" style="width: 100%">
      ...
      <el-table-column label="简介" width="200">
        <template #default="{ row }">
          <div class="ellipsis">{{ row.introduce }}</div>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" width="250">
        <template #default="{ row }">
          ...
          <el-button size="small" type="primary" @click="">查看相关留言</el-button>
          <el-button size="small" type="text" @click="viewBlogContent(row)">查看博客内容</el-button>
        </template>
      </el-table-column>
    ...
    <!-- 弹窗展示博客内容 -->
    <el-dialog v-model="dialogVisible" title="博客内容">
      <div>
        <h3>{{ selectedBlog.title }}</h3>
        <p><strong>简介:</strong> {{ selectedBlog.introduce }}</p>
        <p><strong>内容:</strong> </p>
        <div v-html="selectedBlog.context">
        </div>
      </div>
...
const getBlogInfoList=async ()=>{
  const response= await axios.get("http://localhost:8080/admin/getBlogList", {
    headers: {
  ...
  if(response.data.code===200){
    filteredBlogs.value=response.data.data.pageList.map(item=>{
      return {
        ...item,
        status: statusMapping[item.status] // 转换状态
      };
    })
...
const deleteBlog= async (blogId)=>{
  const response=await axios.delete("http://localhost:8080/admin/deleteBlog",{
    headers:{
      token: localStorage.getItem('token')
    },
    params:{
