<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="commentContent" label="评论内容" width="200" />
...
function formatDate(isoString) {
  ...
  if("1970-01-01 08:00:00"===`${year}-${month}-${day} ${hours}:${minutes}:${seconds}`){
    return "暂无"
  }
...
const handleStatusChange= async(row)=>{
  const response= await axios.put("http://localhost:8080/admin/report/updateCommentReport", {
    commentId:row.id,
    status:row.status,
  },{
    headers: {
...
const getInfoList=async ()=>{
  ...
  const response= await axios.get("http://localhost:8080/admin/report/getCommentReport", {
    headers: {
      token: localStorage.getItem("token"),
    },
    params: {
...
const deleteReport=async(id)=>{
  ...
  const response=await axios.delete("http://localhost:8080/admin/report/deleteCommentReport",{
    headers:{
      token: localStorage.getItem('token')
    },
    params:{
