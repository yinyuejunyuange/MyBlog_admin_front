<template>
  <div>
    ...
    >
      ...
      <el-table-column prop="userReportName" label="被举报者名称" width="200" />
...
const handleStatusChange= async(row)=>{
  const response= await axios.put("http://localhost:8080/admin/report/updateUserReport", {
    reportId:row.id,
    status:row.status,
  },{
    headers: {
...
const getInfoList=async ()=>{
  ...
  const response= await axios.get("http://localhost:8080/admin/report/getUserReport", {
    headers: {
      token: localStorage.getItem("token"),
    },
    params: {
...
const deleteReport=async(id)=>{
  ...
  const response=await axios.delete("http://localhost:8080/admin/report/deleteUserReport",{
    headers:{
      token: localStorage.getItem('token')
    },
    params:{
