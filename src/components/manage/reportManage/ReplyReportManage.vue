...
const handleStatusChange= async(row)=>{
  const response= await axios.put("http://localhost:8080/admin/report/updateReplyReport", {
    reportId:row.id,
    status:row.status,
  },{
    headers: {
...
const getInfoList=async ()=>{
  ...
  const response= await axios.get("http://localhost:8080/admin/report/getReplyReport", {
    headers: {
      token: localStorage.getItem("token"),
    },
    params: {
...
const deleteReport=async(id)=>{
  ...
  const response=await axios.delete("http://localhost:8080/admin/report/deleteReplyReport",{
    headers:{
      token: localStorage.getItem('token')
    },
    params:{
